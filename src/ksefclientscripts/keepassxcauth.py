...
from ksefclientscripts.utils import get_app_datadir
...
class KeepassxcAuth:
    ...
    def connect(self):
        self.connection = self._establish_connection("kpxc_server")
    ...
    def unlock_database(self):
        ...
        if not self.is_database_open():
            ...
            # "is_database_open()" returns False and ask KeePassXC to authenticate again
...
def get_auth_data_direct(db_file_path, entry_title):
    # ask for password on command line
    if not os.path.exists(db_file_path):
        message = f"unable to find file {db_file_path}"
        raise RuntimeError(message)
    kp_db_pass = getpass.getpass(prompt=f"Requesting entry '{entry_title}', enter password for {db_file_path}: ")

    kp = PyKeePass(db_file_path, password=kp_db_pass)
    keepass_entry = kp.find_entries(title=entry_title, first=True)
    if keepass_entry is None:
        message = f"unable to find entry '{entry_title}' in keepass database"
        raise RuntimeError(message)
    return {"login": keepass_entry.username, "password": keepass_entry.password}  # pylint: disable=E1101


# def get_auth_data_from_app(access_url):
#     # ruff: noqa: PLW0603
